...
// WebSocket server (sem express-ws, usa ws diretamente)
...
function startCapture() {
  ...
  ffmpegProc.stdout.on('error', (err) => {
    log(`[ffmpeg] erro saida: ${err.message}`);
  });

  ffmpegProc.on('error', (err) => {
    log(`[ffmpeg] falha ao iniciar: ${err.message}`);
    ffmpegProc = null;
    scheduleReconnect();
  });
  ...
  ffmpegProc.on('close', (code, signal) => {
    ffmpegProc = null;
    if (signal === 'SIGTERM') {
      log('[ffmpeg] encerrado.');
    } else {
      log(`[ffmpeg] encerrou (codigo ${code}). Reconectando em 3s...`);
      scheduleReconnect();
    }
  });
}
...
function scheduleReconnect() {
  if (clients.size > 0) {
    reconnectTimer = setTimeout(startCapture, 3000);
  }
}
...
wss.on('connection', (ws, req) => {
  ...
  // Cabecalho JSMpeg: 'jsmp' + largura + altura (0 = detecta automaticamente)
  ...
  ws.on('close', () => {
    clients.delete(ws);
    log(`Cliente desconectado (${clients.size} restante(s))`);
    if (clients.size === 0) stopCapture();
  });

  ws.on('error', (err) => {
    log(`[ws] erro: ${err.message}`);
    clients.delete(ws);
    if (clients.size === 0) stopCapture();
  });
});
...
app.get('/config', (_req, res) => {
  res.json({
    scriptUrl: JSMPEG_URL,
    wsUrl: `ws://localhost:${PORT}/stream`,
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`  ║  Camera: 192.168.15.17:554           ║`);
  console.log('  ╚══════════════════════════════════════╝');
  console.log('');
